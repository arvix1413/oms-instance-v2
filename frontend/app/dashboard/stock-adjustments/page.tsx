...
export default function StockAdjustmentsPage() {
  ...
  const save = async () => {
    try {
      ...
      toast('庫存調整單建立成功'); setCreating(false)
  ...
  const approve = async (id: number) => {
    ...
    try {
      ...
      if (viewing?.id === id) { const d = await apiFetch<Adj>(`/api/stock-adjustments/${id}`); setViewing(d) }
    } catch (e: any) { toast(e.message, 'error') }
  }

  const del = async (id: number) => {
    if (!await confirmDialog('確定刪除此調整單？')) return
    try { await apiFetch(`/api/stock-adjustments/${id}`, { method: 'DELETE' }); load() }
    catch (e: any) { toast(e.message, 'error') }
  }

  const viewAdj = async (id: number) => { const d = await apiFetch<Adj>(`/api/stock-adjustments/${id}`); setViewing(d) }
  const addItem = () => setForm(p => ({ ...p, items: [...p.items, emptyItem()] }))
  const removeItem = (i: number) => setForm(p => ({ ...p, items: p.items.filter((_, idx) => idx !== i) }))
  const updateItem = (i: number, f: keyof AdjItem, v: any) => setForm(p => ({
    ...p, items: p.items.map((item, idx) => {
      if (idx !== i) return item
      const u = { ...item, [f]: v }
      if (f === 'actual_qty' || f === 'system_qty') u.diff_qty = (f === 'actual_qty' ? Number(v) : u.actual_qty) - (f === 'system_qty' ? Number(v) : u.system_qty)
      return u
    })
  }))
  ...
  return (
    <div>
                  ...
                  <tr key={i} className="border-b border-slate-100">
                    <td className="p-1">
                      <select className={inp} style={{width:130}} value={item.material_code}
                        ...
                        {materials.map(m => <option key={m.material_code} value={m.material_code}>{m.material_code} - {m.material_name}</option>)}
                      </select>
                    </td>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-3xl w-full max-h-[85vh] overflow-auto">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex gap-2 items-center">
                <span className={viewing.status === 'approved' ? 'badge-green' : 'badge-gray'}>{viewing.status === 'approved' ? '已核准' : '草稿'}</span>
                {viewing.status === 'draft' && <button onClick={() => approve(viewing.id)} className="btn-primary">✓ 核准</button>}
      ...
      <div className="mb-4"><input className="oms-input w-64" placeholder="搜尋調整單號..." value={search} onChange={e => setSearch(e.target.value)} /></div>
        ...
        {loading ? <div className="flex justify-center py-16"><div className="w-5 h-5 border-2 border-blue-200 border-t-blue-600 rounded-full animate-spin" /></div> : (
          <>
            <table className="oms-table">
              <thead><tr>
                <th>調整單號</th><th>類型</th><th>調整日期</th><th>備註</th><th>狀態</th><th>操作</th>
              </tr></thead>
              <tbody>
                {paged.map(a => (
                  <tr key={a.id}>
                ...
                {paged.length === 0 && <tr><td colSpan={6} className="text-center py-12 text-slate-400">尚無庫存調整記錄</td></tr>}
              </tbody>
            </table>
