...
type BomItem = { material_code: string; material_name: string; spec: string; unit: string; quantity: number; supplier_name: string; supplier_price: number; company_price: number; currency: string; remark: string }
type Bom = { id: number; product_sku: string; product_name: string; version: string; status: string; created_at: string; items?: BomItem[] }
const emptyItem = (): BomItem => ({ material_code:'', material_name:'', spec:'', unit:'PCS', quantity:1, supplier_name:'', supplier_price:0, company_price:0, currency:'VND', remark:'' })
...
export default function BomPage() {
  ...
  const save = async () => {
    try {
      await apiFetch('/api/bom', { method: 'POST', body: JSON.stringify(form) })
      showMsg('BOM 建立成功')
      setCreating(false)
  ...
  const selectMaterial = (i: number, mat: any) => {
    ...
    setForm(p => ({ ...p, items: p.items.map((item, idx) => idx === i ? {
      ...
      spec: mat.spec || '', unit: mat.unit || 'PCS',
      supplier_name: mat.supplier_name || '',
      supplier_price: mat.supplier_price || 0, company_price: mat.company_price || 0,
      currency: mat.currency || 'VND',
    } : item) }))
  }
    ...
    <div>
      ...
      {creating && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center p-6 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-5xl my-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-semibold text-slate-800">建立 BOM 表</h2>
              <button onClick={() => setCreating(false)} className="text-slate-400 hover:text-slate-600 text-xl leading-none">✕</button>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-3 gap-4 mb-5">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">產品 SKU *</label>
                  <input className="oms-input" placeholder="例：SHOE-001" value={form.product_sku} onChange={e=>setForm(p=>({...p,product_sku:e.target.value}))} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">產品名稱 *</label>
                  <input className="oms-input" placeholder="例：運動鞋 A款" value={form.product_name} onChange={e=>setForm(p=>({...p,product_name:e.target.value}))} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">版本</label>
                  <input className="oms-input" placeholder="V1" value={form.version} onChange={e=>setForm(p=>({...p,version:e.target.value}))} />
                </div>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {['料號（點擊選擇）','材料名稱','規格','單位','數量','供應商','供應商單價','公司售價','幣別','備註',''].map(h => (
                  ...
                  <tbody>
                    {form.items.map((item, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0">
                        ...
                        <td className="p-1.5 min-w-[120px]"><input className={inp} value={item.material_name} onChange={e=>updateItem(i,'material_name',e.target.value)} /></td>
                        <td className="p-1.5 min-w-[80px]"><input className={inp} value={item.spec} onChange={e=>updateItem(i,'spec',e.target.value)} /></td>
                        <td className="p-1.5 w-16"><input className={inp} value={item.unit} onChange={e=>updateItem(i,'unit',e.target.value)} /></td>
                        <td className="p-1.5 w-20"><input type="number" className={inp} value={item.quantity || ""} onChange={e=>updateItem(i,'quantity',Number(e.target.value))} /></td>
                        <td className="p-1.5 min-w-[100px]"><input className={inp} value={item.supplier_name} onChange={e=>updateItem(i,'supplier_name',e.target.value)} /></td>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
              <button onClick={() => setCreating(false)} className="btn-ghost">取消</button>
              <button onClick={save} className="btn-primary">建立 BOM</button>
            </div>
          </div>
        </div>
      )}
      ...
      {viewing && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-xl p-6 max-w-4xl w-full max-h-[80vh] overflow-auto">
            <div className="flex items-start justify-between mb-4">
              ...
              <button onClick={() => setViewing(null)} className="text-slate-400 hover:text-slate-600 text-xl ml-4">✕</button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    {['料號','材料名稱','規格','單位','數量','供應商','供應商單價','公司售價','幣別','備註'].map(h => (
                ...
                <tbody>
                  {(viewing.items || []).map((item, i) => (
                    <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2 text-right text-slate-600">{item.supplier_price.toLocaleString()}</td>
                      <td className="px-3 py-2 text-right text-slate-800 font-semibold">{item.company_price.toLocaleString()}</td>
